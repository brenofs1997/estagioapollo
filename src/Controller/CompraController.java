...
public class CompraController {
    ...
    public List<itens_Compra> addProduto(Produto prod, TableView<itens_Compra> tabProd, int qtde, Double uni, Label lbTotal) {
        int index = 0;
        int qtdeAlter = 0;
        Double total = 0.0;
        NumberFormat nf = new DecimalFormat("#,###.00");
        ...
        if (!tabProd.getItems().isEmpty()) {
            ...
            if (index >= 0) {
                ...
                FXMLTelaCompraController.getItensCompra().add(new itens_Compra(qtde, uni, total, new Compra(), prod));
            }
        } else {
            total = uni * qtde;
            try {
        ...
        return FXMLTelaCompraController.getItensCompra();
    }
    ...
    public boolean alterar(List<ContasPagar> listaParcela, List<itens_Compra> itensCompra, Funcionario func, int cod, JFXComboBox<Fornecedor> cbFornecedor, Date emissao, JFXComboBox<CondicaoPagamento> cbCond, Double total) {
        boolean aux = false;
        ContasPagar cp = new ContasPagar();
        Compra c = new Compra(cod, emissao, cbFornecedor.getSelectionModel().getSelectedItem(), cbCond.getSelectionModel().getSelectedItem(), total);

        if (c.alterar(c) && c.salvarItens(itensCompra)) {
            int codCompra = 0;
            codCompra = cod;
    ...
    public void carregaCampos(Pane pnconteudo, Compra compra, JFXComboBox<CondicaoPagamento> cbCondPgto, JFXComboBox<Fornecedor> cbFornecedor, DatePicker dtemissao, DatePicker dtvenc, JFXTextField txDias, JFXTextField txQuantParc, JFXTextField txcodigo, JFXButton btFinalizar, JFXButton btExcluir, JFXButton btNovo, TableView<itens_Compra> tabelaProd, TableView<ContasPagar> tabelaParcelas, Label lbTotal,JFXButton btRetirarProd) {
        if (compra.verificarParcelaPaga(compra)) {
            ...
            cv.converter(listaParcela.get(0).getVencimentoDate(), dtvenc);
            lbTotal.setText(nf.format(calcTotal()));
    ...
    public boolean excluir(Compra compra) {
        ...
        if (compra != null) {
            ...
            if (cp.apagarParcCompra(compra.getCodigo()) &&  it.apagar(compra.getCodigo()) && compra.apagar(compra.getCodigo())) {
                return true;
            }
        }
        return false;
    ...
    public boolean excluirAlt(Compra compra) {//excluir para alterar
        itens_Compra it = new itens_Compra();
        ContasPagar cp = new ContasPagar();
        if (compra != null) {

            if (it.apagar(compra.getCodigo()) && cp.apagarParcCompra(compra.getCodigo())) {
                return true;
            }
        }
        return false;
