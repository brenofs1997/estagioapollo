 ...
 * @author paulo
 */
public class FXMLTelaContasReceberController implements Initializable {
    ...
    private void Estornar(ActionEvent event) {
        ...
        if (a.showAndWait().get() == ButtonType.OK) {
            controller.estornarR(tabelaRec.getSelectionModel().getSelectedItem(), btEstornar);
            consultar();
    ...
    private void Baixar(ActionEvent event) throws IOException {
        if (tabelaRec.getSelectionModel().getSelectedItem() != null) {
            Alert a = new Alert(Alert.AlertType.CONFIRMATION);
            a.setContentText("Confirma a baixa?");
            if (a.showAndWait().get() == ButtonType.OK) {
                controller.baixarR(tabelaRec.getSelectionModel().getSelectedItem());
                tabelaRec.getItems().clear();
                controller.pesquisaRadioR("", tabelaRec);
            }
    ...
    public void consultar() {
        ...
        controller.consultarR(cliente, dtinicial, dtfinal, tabelaRec, status);
    }
    ...
    private void pesqQuitado(ActionEvent event) {
        if (rbQuitados.isSelected()) {
            tabelaRec.getItems().clear();
            controller.pesquisaRadioR("Q", tabelaRec);
    ...
    private void pesqParcial(ActionEvent event) {
        if (rbPendente.isSelected()) {
            tabelaRec.getItems().clear();
            controller.pesquisaRadioR("A", tabelaRec);
        }
    }
